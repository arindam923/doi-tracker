...
$stmt = $pdo->prepare("
    SELECT c.click_id, c.clicked_at, p.project_code, gv.vendor_name, c.ip_address,
           c.country_code, c.device_type, c.browser, c.os, c.isp, c.is_converted,
...
fputcsv($out, ['Click ID', 'Date', 'Project', 'Vendor', 'IP', 'Country', 'Device', 'Browser', 'OS', 'ISP', 'Status', 'Sub1', 'Sub2', 'Sub3', 'Sub4', 'Sub5', 'Referrer']);
while ($r = $stmt->fetch()) {
    fputcsv($out, [
